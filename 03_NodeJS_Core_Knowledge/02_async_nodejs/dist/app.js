...
import { getGeoLocation, getGeoLocationWithCallback, } from './geocoding.js';
// CONSTANTS & VARIABLES
const space = '\n';
// FUNCTIONS
const yargsArgv = () => {
    ...
    cmd.command({
        ...
        handler: async (argv) => {
            try {
                ...
                getGeoLocationWithCallback(argv.name, async (error, geoCallbackData = {}, 
                ...
                { lat, lng } = {}) => {
                    if (error)
                        ...
                        const weatherData = await getWeatherData(geoCallbackData.lat, geoCallbackData.lng);
                        console.log(space, chalk.blueBright.inverse('Callback Weather Data'), space, weatherData);
                    }
                });
                const goeData = await getGeoLocation(argv.name);
                console.log(space, chalk.blueBright.inverse('Location Geo-Coordinates Detail'), space, goeData);
                if ('lat' in goeData) {
                    const weatherData = await getWeatherData(goeData.lat, goeData.lng);
                    console.log(space, chalk.blueBright.inverse('Weather Data'), space, weatherData);
                }
            }
            catch (err) {
...
const destructuring = () => {
    ...
    const transaction = (type, { label, stock }) => {
        console.log(space, 'Type: ', type, space, 'Label: ', label, space, 'Stock: ', stock);
    };
