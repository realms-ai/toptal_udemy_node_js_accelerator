...
// FUNCTIONS
const processArgv = () => {
  const greetMsg: string = chalk.green.inverse.bold('Success');
...
const yargsArgv = () => {
  ...
  cmd.command({
    ...
    builder: {
      title: {
        describe: 'Note title',
        demandOption: true, // provide this option to force user to input title
...
const arrowFunctions = () => {
  const event = {
    name: 'Navpreet Singh',
    printGuestList: function (): void {
      console.log(
        chalk.green.inverse.bold('(Old way) Guest list for ', this.name)
      );
    },
    printGuestList_1(): void {
      console.log(
        chalk.green.inverse.bold('(New way) Guest list for ', this.name)
      );
    },
    printGuestList_2: (): void => {
      console.log(
        chalk.green.inverse.bold('(Arrow function) Guest list for ', event.name)
      );
    },
  };
  console.log(space);
  ...
  const tasks: { tasks: Task[]; getTasksToDo: () => Task[] } = {
    tasks: [
      {
  ...
  console.log(
    space,
    chalk.yellow.inverse('Tasks to do'),
    tasks.getTasksToDo(),
    space
  );
};
...
const task1 = () => {
  ...
  try {
    ...
    if (typeof data === 'object') {
      ...
      let dataToBeModified: JsonData = { ...data[1] };
