...
const createUser = async ({ userInput }: any, req: any) => {
  ...
  try {
    ...
    if (!emailValidation) errors.push({ message: 'Email is invalid' });
    if (passwordValidation.includes(false))
      errors.push({ message: 'Password too short' });

    if (errors.length > 0) {
      errLog('Errors: ', errors);
      const error: Error = new Error('Invalid Input');
      error.cause = {
        data: errors,
        code: 422,
      };
...
const login = async ({ userInput }: any, req: Request) => {
...
const createPost = async ({ userInput }: any, req: Request) => {
