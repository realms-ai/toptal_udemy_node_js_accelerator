...
import { CartItem, CartItemType } from '../models/cart-item.js';
import { OrderItemType } from '../models/order-item.js';
...
const show = () => {
  router.get('/:id', async (req, res) => {
    ...
    if (order) orderItems = await getItems(order);
    res.render('orders/index', {
      headTitle: 'Orders',
      path: 'orders',
      domain: domain,
...
const create = () => {
  router.post('/', async (req, res) => {
    ...
    const cartProducts: Product[] = await cart.getProducts();
...
const getUserOrders = async (user: User, orderId?: number) => {
...
const getItems = async (order: Order) => {
