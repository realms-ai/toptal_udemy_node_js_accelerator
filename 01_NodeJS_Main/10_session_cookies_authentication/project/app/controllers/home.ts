...
const { domain, SENDGRID_API_KEY } = Constants;
...
const signup = () => {
  ...
  const emailValidation = check('email')
    ...
    .custom(async (value, { req }) => {
      ...
      if (user) throw new Error('This email address is already taken');
    });
  const passwordValidation = body(
    'password',
    'Password should be minimum 5 chars long'
  )
    .isLength({ min: 5 })
    .isAlphanumeric();
  const confirmPasswordValidation = body('confirmPassword').custom(
    (value, { req }) => {
    ...
    async (req, res) => {
      try {
        ...
        if (!errors.isEmpty()) {
          ...
          return res.status(422).render('home/signup', {
            headTitle: 'signup',
            path: 'signup',
            domain: domain,
            // loggedIn: req.session?.isLoggedIn,
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (user) {
        ...
        // Save session before redirect to make sure session is stored in DB
        // await req.session.save();
      } else {
        req.flash('error', 'Invalid User');
        res.redirect(`${domain}/login`);
